...
// 2. Refactor openProofModal & save-proof-btn (approx line 872)
code = code.replace(
...
code = code.replace(/state\.donations\.push\(\{\s*(.*?)\s*\}\);\s*saveState\(\);/gs, (match, p1) => {
    return "const newItem = { " + p1 + " }; fbSet('donations/' + newItem.id, newItem);";
});
code = code.replace(/state\.announcements\.push\(\{\s*(.*?)\s*\}\);\s*saveState\(\);/gs, (match, p1) => {
    return "const newItem = { " + p1 + " }; fbSet('announcements/' + newItem.id, newItem);";
});
code = code.replace(/state\.events\.push\(\{\s*(.*?)\s*\}\);\s*saveState\(\);/gs, (match, p1) => {
...
console.log('Done refactoring app.js');
